...
import { Thread, useStore } from '../../stores/store'
...
const item = ref<Thread>(props.thread)
const config = {
  headers: {
    Authorization: 'Bearer ' + localStorage.getItem('token')
  }
}
const markdownBody = ref<string>(localStorage.getItem('theme') === 'dark' ? 'github-markdown-body' : 'vuepress-markdown-body')
...
const themeChange = (theme: 'light' | 'dark') => {
  markdownBody.value = theme === 'dark' ? 'github-markdown-body' : 'vuepress-markdown-body'
}
...
const likeThread = (index: any) => {
  if (!item.value.likeFlag) {
    createThreadLikeApi(item.value.id, config).then(res => {
      console.log(res)
      if (res.data.code === 200) {
        item.value.likeFlag = true
        item.value.likeNum++
      }
    })
  } else {
    deleteThreadLikeApi(item.value.id, config).then(res => {
      console.log(res)
      if (res.data.code === 200) {
        item.value.likeFlag = false
        item.value.likeNum--
      }
    })
  }
...
const favThread = (index: any) => {
  if (!item.value.favoriteFlag) {
    createThreadFavApi(item.value.id, config).then(res => {
      if (res.data.code === 200) {
        item.value.favoriteFlag = true
        item.value.favNum++
      }
    })
  } else {
    deleteThreadFavApi(item.value.id, config).then(res => {
      if (res.data.code === 200) {
        item.value.favoriteFlag = false
        item.value.favNum--
      }
    })
  }
...
<template>
  <div class="mt-8">
    ...
    <div class="flex justify-start items-center">
      <img :src="`http://icon.mgaronya.com/${item.authorIcon}`" class="h-8 w-8 rounded-full shadow-mg hover:cursor-pointer" alt="" @click.stop="toThreadUserpage(item.user_id)" />
      <span class="font-bold px-2 text-lg hover:text-blue-500 hover:cursor-pointer" @click.stop="toThreadUserpage(item.user_id)">
        {{ item.author }}
      </span>
    </div>
    <!-- thread -->
    <div class="mt-4">
      <v-md-preview-html :html="item.res_long" @click="toDetail" class="rounded bg-gray-100 dark:bg-black hover:shadow-md hover:cursor-pointer transition-all duration-500 ease-in-out" :preview-class="markdownBody"> </v-md-preview-html>
      <!-- footer -->
      <div class="likefav h-8 flex justify-start items-center text-gray-500">
        <span class="ml-2 flex items-center justify-center">
          <span @click.stop="likeThread">
            <Transition mode="out-in" enter-active-class="animate_animated animate__heartBeat">
              <svg v-if="!item.likeFlag" t="1679044901705" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6768" width="16" height="16">
                <path
              ...
              <svg v-else t="1679044901705" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6768" width="16" height="16">
                <path
        ...
        <span class="h-6 ml-2 flex items-center">
          <span @click.stop="favThread">
            <Transition mode="out-in" enter-active-class="animate_animated animate__heartBeat">
              <svg v-if="!item.favoriteFlag" t="1679043184932" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5674" width="16" height="16">
                <path
              ...
              <svg v-else t="1679043184932" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5674" width="16" height="16">
                <path
