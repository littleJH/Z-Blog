...
import { userinfoApi, userArticlesApi, getArticlesApi } from '../../api/user'
import { getArticleLabelApi } from '../../api/label'
import {
  getArticleLikedApi,
  getArticleLikeListApi,
  createArticleLikeApi,
...
import { Article } from '../../stores/index'
...
const assign = async (articleRes: any) => {
  const list = reactive<Array<Article>>([])
  for (let item of articleRes.data.data.articles) {
    ...
    await Promise.all([
      ...
      getArticleLabelApi(item.id, config),
      getArticleLikedApi(item.id, config),
      getArticleLikeListApi(item.id, config),
      getArticleFavoritedApi(item.id, config),
      getArticleFavListApi(item.id, config)
    ]).then(result => {
...
const doLike = (index: any) => {
  if (!articleList[index].likeFlag) {
    console.log(articleList[index].id)
    createArticleLikeApi(articleList[index].id, config).then(res => {
      console.log(res)
      if (res.data.code === 200) {
        articleList[index].likeFlag = true
      }
    })
  } else {
    deleteArticleLikeApi(articleList[index].id, config).then(res => {
      if (res.data.code === 200) {
        articleList[index].likeFlag = false
      }
    })
  }
}
...
const doFav = (index: any) => {
  if (!articleList[index].favoriteFlag) {
    createArticleFavApi(articleList[index].id, config).then(res => {
      if (res.data.code === 200) {
        articleList[index].favoriteFlag = true
      }
    })
  } else {
    deleteArticleFavApi(articleList[index].id, config).then(res => {
      if (res.data.code === 200) {
        articleList[index].favoriteFlag = false
      }
    })
  }
}
...
const toDetail = (index: number) => {
  localStorage.setItem('article', JSON.stringify(articleList[index]))
  router.push({
    name: 'article',
    params: {
...
const mouseoverLikeSvg = (index: number) => {
  articleList[index].likeSvgColor = '#1296db'
}

const mouseleaveLikeSvg = (index: number) => {
  articleList[index].likeSvgColor = '#6B7280'
}

const mouseoverFavSvg = (index: number) => {
  articleList[index].favSvgColor = '#1296db'
}

const mouseleaveFavSvg = (index: number) => {
  articleList[index].favSvgColor = '#6B7280'
}
...
const toUserpage = (index: number) => {
  router.push({
    ...
    params: {
      userId: articleList[index].user_id
    }
  })
}
</script>
...
<template>
  ...
  <div class="max-w-screen-md">
    <div
      class="h-auto overflow-hidden bg-white dark:text-white dark:bg-deepBlack"
    ...
    >
      ...
      <div class="p-4">
        ...
        >
          ...
          <p :id="item.id" class="mt-2 truncate"></p>
        </div>
        <!-- footer -->
