...
onBeforeMount(async () => {
  nprogress.start()
  await Promise.all([
    getArticleHotRankingApi(config, 1, topNum),
    getPostHotRankingApi(config, 1, topNum),
    getUserHotRankingJApi(config, 1, topNum)
  ]).then(res => {
...
const toDetail = async (
  mode: 'articleRank' | 'postRank' | 'userRank',
  index: number
) => {
  nprogress.start()
  switch (mode) {
...
<template>
  <div class="ct-bg">
    <!-- rank -->
    <div
      class="max-w-screen-lg rounded shadow-md bg-white grid grid-cols-3 divide-x py-2 dark:bg-deepBlack"
      style="width: 1024px; height: 180px"
    >
      <div class="py-2 px-4">
        <p class="rankHeader">
          <span class="flex-grow font-bold">文章top{{ topNum }}</span>
          <span
            @click="toTopall('article')"
            class="text-xs text-gray-500 flex items-center hover:cursor-pointer hover:underline"
          >
            <svg
              t="1681362064271"
              class="icon"
        ...
        <div class="">
          <TransitionGroup name="list">
            <p
              ...
              v-for="(item, index) in articleRank.articleList"
              :key="index"
            >
              <span
                ><svg
              ...
              <span class="truncate flex-grow">
                <span
                  class="hover:text-blue-500 hover:underline hover:cursor-pointer transition-all ease-in-out"
                  v-if="item"
                  @click="toDetail('articleRank', index)"
                  >{{ item.title }}</span
                >
              ...
              <span class="flex items-center ml-2">
                ...
                <span class="text-sm">{{
                  Math.trunc(articleRank.articles[index].Score)
                }}</span>
              </span>
            </p>
          </TransitionGroup>
      ...
      <div class="py-2 px-4">
        ...
        <TransitionGroup name="list">
          <p
            v-for="(item, index) in postRank.postList"
            :key="index"
            class="flex justify-between py-1"
          >
            <span>
            ...
            <span class="flex items-center ml-2">
              ...
              <span class="text-sm">{{
                Math.trunc(postRank.posts[index].Score)
              }}</span>
            </span>
          </p>
        </TransitionGroup>
      ...
      <div class="py-2 px-4">
        ...
        <TransitionGroup name="list">
          <p
            v-for="(item, index) in userRank.userList"
            :key="index"
            class="flex justify-between py-1"
          >
            <span>
            ...
            <span
              @click="toDetail('userRank', index)"
              class="truncate flex-grow hover:text-blue-500 hover:underline hover:cursor-pointer transition-all ease-in-out"
              >{{ item.Name }}</span
            >
            <span class="flex items-center ml-2">
              <svg
              ...
              <span class="text-sm">{{
                Math.trunc(userRank.users[index].Score)
              }}</span>
            </span>
          </p>
        </TransitionGroup>
    ...
    <!-- recomment -->
    <div
      class="mt-8 max-w-screen-lg rounded shadow-md bg-white p-4 dark:bg-deepBlack"
      style="width: 1024px; min-height: 400px"
    >
      <div class="flex justify-start">
        ...
        <div class="navBar ml-4">
          <span
            :class="{ activeNav: currentCom === 'Articles' }"
            @click="currentCom = 'Articles'"
            >文章</span
          >
          <span
            :class="{ activeNav: currentCom === 'Posts' }"
            @click="currentCom = 'Posts'"
            >帖子</span
          >
          <span
            :class="{ activeNav: currentCom === 'Users' }"
            @click="currentCom = 'Users'"
            >用户</span
          >
        </div>
      </div>
